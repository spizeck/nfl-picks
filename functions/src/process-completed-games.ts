...
export const processCompletedGames = onCall(async (request) => {
  ...
  try {
    ...
    const allUsersStats = new Map<string, UserStats>();
    ...
    for (const [userId, stats] of allUsersStats) {
      ...
      console.log(
        `Updated week ${week} stats for user ${userId}: ` +
          `${stats.wins}-${stats.losses}-${pending}`
      );
    }
    ...
    return {
      success: true,
      message:
        `Processed ${gamesSnapshot.size} completed games ` +
        `for ${usersSnapshot.size} users`,
